...
public class UploadService {
	...
	/**
	 * 生成规范的文件名 accountId_年月日时分秒.jpg 包含 accountId
	 * 以便于找到某人上传的图片，便于定位该用户所有文章，方便清除恶意上传 图片中添加一些 meta
	 * 信息：accountId_201604161359.jpg 目录中已经包含了模块名了，这里的 meta 只需要体现 accountId
	 * 与时间就可以了
	 */
	 ...
	 * 
	 * @param uploadType
	 *            上传类型，目前支持四种：project, share, feedback, document
	 */
	private void buildPathAndFileName(String uploadType, Integer accountId, String extName,
			String[] relativePathFileName, String[] absolutePathFileName, String[] fileName) {
	...
	/**
	 * 目前使用 File.renameTo(targetFileName) 的方式保存到目标文件， 如果 linux 下不支持，或者将来在 linux
	 * 下要跨磁盘保存，则需要 改成 copy 文件内容的方式并删除原来文件的方式来保存
	 */
